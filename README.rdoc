...
Since its fixing problems and bugs it should be optional in standard compliant
browsers.
...
- manually extend the attributes with <tt>$(element.parentNode)</tt> or
  <tt>Alpha.extendElement(element.parentNode)</tt>.
- use the Element.prototype.get() method of elements, which returns extended elements.
...
Internet Explorer before IE9 lacks support for DOM Events, and still relies
