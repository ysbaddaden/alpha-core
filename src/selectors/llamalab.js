...
//      bcn  = !!d.getElementsByClassName,
  ...
  */
  ...
  {
    var elements = Selector(cssRule, this);
    if (Alpha.extendElements) {
      elements = Alpha.extendElements(elements);
    }
    return new Alpha.NodeList(elements);
  }
  ...
  Element.prototype.querySelector = function(cssRule)
  {
    var elements = Selector(cssRule, this);
    if (elements.length) {
      return Alpha.extendElement ? Alpha.extendElement(elements[0]) : elements[0];
    }
  }
  
  document.querySelectorAll = function(cssRule)
  {
    var elements = Selector(cssRule, document);
    if (Alpha.extendElements) {
      elements = Alpha.extendElements(elements);
    }
    return new Alpha.NodeList(elements);
